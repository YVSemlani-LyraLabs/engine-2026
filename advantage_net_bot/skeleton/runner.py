        ...
        for packet in self.receive():
            for clause in packet:
                if clause[0] == 'T':
                    ...
                    if hasattr(self.pokerbot, 'reset_action_history'):
                        self.pokerbot.reset_action_history()
                elif clause[0] == 'G':
                    ...
                    action = FoldAction()
                    if hasattr(self.pokerbot, 'observe_action') and isinstance(round_state, RoundState):
                        self.pokerbot.observe_action(round_state, action)
                    ...
                    if isinstance(round_state, RoundState):
                        action = DiscardAction(int(clause[1:]))
                        if hasattr(self.pokerbot, 'observe_action'):
                            self.pokerbot.observe_action(round_state, action)
                        round_state = round_state.proceed(action)
                    else:
                        pass
                elif clause[0] == 'R':
                    action = RaiseAction(int(float(clause[1:])))
                    if hasattr(self.pokerbot, 'observe_action') and isinstance(round_state, RoundState):
                        self.pokerbot.observe_action(round_state, action)
                    round_state = round_state.proceed(action)
                elif clause[0] == 'B':
